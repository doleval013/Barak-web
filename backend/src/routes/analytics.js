/**
 * Analytics Routes — Visit tracking & stats
 * 
 * Extracted from the original backend/index.js monolith.
 * All existing analytics functionality is preserved.
 * 
 * Public:
 *   POST /api/visit            — Record page visit
 *   POST /api/visit/heartbeat  — Update session heartbeat
 *   POST /api/visit/scroll     — Update scroll depth
 *   POST /api/event            — Record analytics event
 * 
 * Protected (Admin):
 *   GET  /api/stats            — Get all analytics stats
 */
...
const { authenticateAdminLegacy } = require('../middleware/auth');
...
module.exports = (pool) => {
    ...
    // 2. Heartbeat (Public)
    router.post('/visit/heartbeat', async (req, res) => {
    ...
    router.post('/visit/scroll', async (req, res) => {
    ...
    // 4. Get Stats (Protected — supports both JWT admin and legacy ADMIN_HASH)
    router.get('/stats', authenticateAdminLegacy(pool), async (req, res) => {
        try {
            const stats = {};
