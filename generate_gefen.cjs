...
const original = fs.readFileSync('src/components/TeamWorkshopLanding.jsx', 'utf8');

// The new translations we want to insert
...
// 1. We want code perfectly up to `const translations = {`
const startIdx = original.indexOf('const translations = {');

// 2. We want code starting strictly from `/* ============================================================ \n   MAIN COMPONENT`
const mainCompIdx = original.indexOf('/* ============================================================\r\n   MAIN COMPONENT');
const mainCompFallback = original.indexOf('/* ============================================================\n   MAIN COMPONENT');
const endIdx = mainCompIdx !== -1 ? mainCompIdx : mainCompFallback;

if (startIdx === -1 || endIdx === -1 || endIdx <= startIdx) {
    console.log("Could not locate translation or main component blocks correctly");
    process.exit(1);
}

let modified = original.substring(0, startIdx) + newTranslations + "\n\n" + original.substring(endIdx);

// Standard substitutions
modified = modified.replace(/function TeamWorkshopLanding\(\)/g, 'function GefenLanding()');
modified = modified.replace(/export default TeamWorkshopLanding;/g, 'export default GefenLanding;');
modified = modified.replace(/_subject: 'מתעניין בסדנת כלבנות לפיתוח צוות'/g, '_subject: \'מתעניין בתוכנית גפ"ן\'');
modified = modified.replace(/type: 'form_submit', name: 'workshop_lead'/g, 'type: \'form_submit\', name: \'gefen_lead\'');

// Image override in CSS hero class
modified = modified.replace('className="workshop-hero workshop-hero--image-bg"', 'className="workshop-hero" style={{ backgroundImage: `url(${imgGate})`, backgroundSize: "cover", backgroundPosition: "center 30%" }}');

// Replace standard image imports
modified = modified.replace("import imgFeed from '../assets/workshop/feed.jpg';", "import imgFeed from '../assets/schools/kid-feed-jessie.jpg';");
modified = modified.replace("import imgGate from '../assets/workshop/gate.jpg';", "import imgGate from '../assets/schools/jessie-petted.jpg';");
modified = modified.replace("import imgHighfive from '../assets/workshop/highfive.jpg';", "import imgHighfive from '../assets/schools/jessie-and-soccor.jpg';");
modified = modified.replace("import imgRelax from '../assets/workshop/relax.jpg';", "import imgRelax from '../assets/schools/jessie-petted-zoomin.jpg';");
modified = modified.replace("import imgRelax2 from '../assets/workshop/relax2.jpg';", "import imgRelax2 from '../assets/schools/girl-feed-jessie.jpg';");

// Replace YouTube link
modified = modified.replace(/kZMeB9DZNAs/g, 'HC4Sm4KhXlU');

fs.writeFileSync('src/components/GefenLanding.jsx', modified);
console.log("GefenLanding successfully generated.");
